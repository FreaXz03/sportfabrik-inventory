...
def _reduktionen(session, lagerort_id: int, heute: date) -> dict:
    """Artikel mit Bestand, deren nächste Reduktionsstufe fällig ist oder in
    den nächsten `VORSCHAU_TAGE` Tagen fällig wird (Regel 6: 18 Monate → 50 %,
    36 Monate → 70 %)."""
    eingaenge = _letzte_eingaenge(lagerort_id)
    mit_bestand = (
        select(Variante.artikel_id)
    ...
    daten = session.execute(
        select(eingaenge.c.artikel_id, eingaenge.c.datum).where(
            eingaenge.c.artikel_id.in_(select(mit_bestand.c.artikel_id))
        )
    ).all()
    ergebnis = {}
    for monate, prozent in sorted(STUFEN):  # 18 → 50 %, dann 36 → 70 %
        ...
        faellig = sum(
            1
            for _, datum in daten
            if datum <= grenze_heute and (obergrenze is None or datum > obergrenze)
        )
        bald = sum(1 for _, datum in daten if grenze_heute < datum <= grenze_bald)
        ergebnis[str(prozent)] = {"faellig": faellig, "bald": bald}
    return ergebnis
...
def stamm(session) -> dict:
    ...
    return {
        "ohne_kategorie": int(
            session.scalar(select(func.count()).select_from(Artikel).where(Artikel.kategorie_id.is_(None))) or 0
        ),
        "ohne_ean": int(
