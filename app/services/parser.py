    ...
    with document:
        ...
        for page_index, page in enumerate(document):
            ...
            body = [
                r
                for r in rows
                if top < r[0][1] < stop and max(w[3] - w[1] for w in r) > 3
            ]
            current = None
            page_items = []
            for row in body:
...
if __name__ == "__main__":
    cli = argparse.ArgumentParser(
        description="INTERSPORT-PDF als JSON-Vorschau auslesen"
    )
    cli.add_argument("pdf", type=Path)
    cli.add_argument("--output", type=Path)
    args = cli.parse_args()
    result = json.dumps(
        parse_invoice(args.pdf.read_bytes()), ensure_ascii=False, indent=2
    )
    if args.output:
