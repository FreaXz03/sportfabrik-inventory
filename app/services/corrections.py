...
def apply_corrections(parsed, corrections=None, actor=None):
    ...
    if set(patches) - known:
        raise CorrectionError(
            "Eine korrigierte Position existiert nicht in der Rechnung."
        )
    for item in result["items"]:
        patch = patches.get(str(item["row_number"]), {})
        if not isinstance(patch, dict) or set(patch) - FIELDS.keys():
            raise CorrectionError(
                "Nur die vorgesehenen Artikelfelder dürfen korrigiert werden."
            )
        original = deepcopy(item)
        for key, value in patch.items():
            if not isinstance(value, str) or len(value) > FIELDS[key]:
                raise CorrectionError(
                    f'Position {item["row_number"]}: ungültiger Wert für {key}.'
                )
        ...
        for warning in original["warnings"]:
            if warning.startswith(
                (
                    "Pflichtfeld fehlt:",
                    "EAN hat ein unerwartetes",
                    "Ungültiger Wert für",
                )
            ):
                continue
            if warning.startswith("Farbe/Grösse nicht eindeutig") and all(
        ...
        for key in ("quantity", "uvp"):
            try:
                ...
                errors.append(
                    f"{key}: gültige Zahl mit höchstens zwei Nachkommastellen erforderlich."
                )
