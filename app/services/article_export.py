...
def export_articles(items):
    ...
    sheet.title = 'Artikel'
    sheet.append(['Marke', 'Bezeichnung', 'Lief. Art. Nr.', 'EAN', 'Farbe', 'Grösse', 'Geliefert gesamt', 'Einheit', 'Letzter UVP (CHF)', 'UVP vom', 'Erste Lieferung', 'Letzte Lieferung'])
    for item in items:
        ...
        for delivered in item['delivered'] or [{'quantity': None, 'unit': None}]:
            values = [item[k] for k in ['brand', 'description', 'supplier_article_no', 'ean', 'color', 'size']]
            ...
            sheet.cell(sheet.max_row, 9).number_format = '0.00 "CHF"'
    ...
    for index, width in enumerate([20, 48, 22, 22, 30, 16, 20, 12, 22, 18, 18, 18], 1):
