...
class Element {
 ...
 addEventListener(){} setAttribute(){} add(node){this.children.push(node);} querySelector(){return new Element();}
}
...
"""
    result=subprocess.run([node,'-'],input=(setup+script+checks).encode(),capture_output=True,timeout=20)
