...
def _tesseract_row(words, block=1, par=1, line=1, top=100, height=30, gap=20, left_start=50):
    ...
    data = {'text': [], 'block_num': [], 'par_num': [], 'line_num': [],
            'left': [], 'top': [], 'width': [], 'height': [], 'word_num': []}
    left = left_start
...
def _merge(*rows):
    merged = {'text': [], 'block_num': [], 'par_num': [], 'line_num': [],
              'left': [], 'top': [], 'width': [], 'height': [], 'word_num': []}
    for row in rows:
        for key in merged:
...
def test_grouped_words_converts_pixels_to_points():
    ocr_top = 600
    data = {'text': ['Test'], 'block_num': [1], 'par_num': [1], 'line_num': [1],
            'left': [300], 'top': [ocr_top], 'width': [len('Test') * 18],
            'height': [30], 'word_num': [1]}
    words = ocr._grouped_words(data)
...
def _draw_scanned_invoice_pdf():
    ...
    column_order = ['Marke', 'FEDAS', 'Lief.', 'Art.1', 'Art.2', 'EAN',
                    'Bezeichnung', 'Menge', 'Einheit', 'UVP', 'Preis']
    header_text = {label: ('Art.' if label.startswith('Art.') else label) for label in column_order}
    row = {'Marke': 'Nike', 'Lief.': '12345', 'Art.2': '9988770010',
           'EAN': '4006632041234', 'Bezeichnung': 'Laufschuh Air Zoom',
           'Menge': '24', 'Einheit': 'Stk', 'UVP': '129,99'}
    ...
    for label in column_order:
        columns[label] = x
        widest = max(scratch.textlength(header_text[label], font=font),
...
@pytest.mark.skipif(not TESSERACT_AVAILABLE, reason='Tesseract OCR ist auf diesem Rechner nicht installiert')
