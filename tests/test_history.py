...
def test_invoice_list(client):
    ...
    assert client.get('/api/invoices?page_size=1&page=2').json()['items'][0]['invoice_number']=='old'
...
def test_history_order_and_paging(client):
    ...
    assert client.get(url+'?page_size=1&page=2').json()['items'][0]['quantity']=='3.00'
...
def test_snapshot_preserved(client):
    ...
    try:
        ...
        session.add(InvoiceItemSource(item_id=item.id,data={'description':'Originalbezeichnung',
            'row_number':7,'page':2,'raw_lines':['Originaltext']}))
        session.commit()
