...
def migration_env():
    env = os.environ.copy()
    for key in ('DB_HOST', 'DB_PASSWORD', 'DB_PORT', 'DB_NAME', 'DB_USER', 'DATABASE_URL'):
        env.pop(key, None)
    return env
...
def test_container_url_offline_migrations():
    env = migration_env()
    env.update(DB_HOST='db', DB_PASSWORD='test@pass%with/slash:#', DB_USER='inventory', DB_NAME='inventory_db')
...
def test_online_migration_to_isolated_database(tmp_path):
    ...
    with sqlite3.connect(tmp_path / 'migration.db') as db:
        assert 'ocr_used' in [row[1] for row in db.execute('PRAGMA table_info(invoices)')]
...
"""
    result = subprocess.run([node, '-'], input=script.encode(), capture_output=True, timeout=15)
    assert result.returncode == 0, result.stderr.decode()
    html = (ROOT / 'app/templates/login.html').read_text(encoding='utf-8')
    assert html.index('src="/static/js/login-redirect.js"') < html.index('safeLoginRedirect(params')
