...
def test_corrected_import_preserves_original_and_actor(setup_import):
    ...
    import_invoice(pdf,'test.pdf',digest,sessions,actor,{'1':{'quantity':'2,50','description':'Korrigiert'}})
    with sessions() as s:
        ...
        assert str(s.scalar(select(InvoiceItem).order_by(InvoiceItem.id)).quantity)=='2.50'


@pytest.mark.parametrize('patch',[{'1':{'id':'3'}},{'9999':{'ean':'12345678'}},{'1':{'ean':123}}])
...
@pytest.mark.parametrize('patch',[{'quantity':'NaN'},{'uvp':'-1'},{'quantity':'1.234'},{'ean':'abc'},{'brand':''}])
