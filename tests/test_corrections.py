...
def test_corrected_import_preserves_original_and_actor(setup_import):
    ...
    with sessions() as s:
        source = s.scalar(
            select(WareneingangPositionQuelle).order_by(WareneingangPositionQuelle.position_id)
        )
        ...
        assert (
            str(s.scalar(select(WareneingangPosition).order_by(WareneingangPosition.id)).menge)
            == "2.50"
        )
...
def test_invalid_values_block_import(setup_import, patch):
    ...
    with pytest.raises(ImportRejected):
        import_invoice(pdf, "test.pdf", digest, sessions, sf1_id, corrections={"1": patch})
    with sessions() as s:
