...
def backup(external=None):
    ...
    command = [docker, 'compose', '--env-file', str(ROOT / '.env.server'), 'exec', '-T', 'db']
    ...
    with tempfile.TemporaryDirectory(prefix='.pending-', dir=destination) as temporary:
        ...
        with dump.open('wb') as output:
            result = subprocess.run(command + ['pg_dump', '-U', 'inventory', '-d', 'inventory_db', '-Fc'],
                cwd=ROOT, stdout=output, stderr=subprocess.PIPE, timeout=1800)
        if result.returncode or dump.stat().st_size == 0:
            raise RuntimeError('Datenbanksicherung fehlgeschlagen: ' + result.stderr.decode(errors='replace'))
        with dump.open('rb') as source:
            result = subprocess.run(command + ['pg_restore', '--list'], cwd=ROOT,
        ...
        pdf_count = 0
        with zipfile.ZipFile(folder / 'original-pdfs.zip', 'w', zipfile.ZIP_DEFLATED) as archive:
        ...
        hashes = {path.name: sha256(path) for path in folder.iterdir() if path.is_file()}
        manifest = {'created_utc': stamp, 'database': 'inventory_db', 'pdf_count': pdf_count,
                    'sha256': hashes, 'validation': 'pg_restore --list and ZIP CRC; no full restore performed'}
        (folder / 'manifest.json').write_text(json.dumps(manifest, indent=2), encoding='utf-8')
        folder.rename(final)
    if external:
        target_root = Path(external)
        if not target_root.is_dir():
            raise RuntimeError(f'Lokales Backup gespeichert: {final}. Externes Ziel nicht erreichbar.')
        ...
        try:
            ...
            raise RuntimeError(f'Lokales Backup gespeichert: {final}. Externe Kopie fehlgeschlagen: {exc}') from exc
    return {'backup': str(final), 'pdf_count': pdf_count, 'external_copy': bool(external)}
